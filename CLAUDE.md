...
**Enforcement lives elsewhere, deliberately.** This pin makes `format: uri` documentation-only for Python, same as every other consumer — it is not where malformed streaming URLs get caught. That happens at the BS boundary guard (`sanitizeLookupStreamingUrls`, WXYC/Backend-Service#2350) and the LML writer seam (`streaming_links` read seam, WXYC/library-metadata-lookup#1295), both of which suppress a malformed value to `null` rather than raising — matching the WXYC/Backend-Service#1714 and #2345 suppress-to-null policy. **Both Python consumers (library-metadata-lookup, request-o-matic) still need the same pin in their own, currently unmigrated `scripts/generate_api_models.sh`** as part of their own post-#428 regen PRs — this script's pin only protects a caller of *this* canonical script, and per the Output locations and consumers table above, neither Python service has migrated onto it yet.
