...
| `generate:swift` | `generated/swift/` | **wxyc-ios-64** | wxyc-ios-64 vendors this output directly into `Shared/WXYCAPIModels/` (WXYC/wxyc-ios-64#598), regenerated via `scripts/regenerate-api-types.sh` against a pinned commit; wxyc-dj-ios still hand-authors its DTOs (`Packages/WXYCAPI/Sources/WXYCAPI/DTOs/`) until its adoption PR (WXYC/wxyc-dj-ios#79) lands |
...
For Kotlin, and still for `wxyc-dj-ios`'s Swift DTOs, the hand-authored types live in the consumer repo (e.g. `wxyc-dj-ios`'s `Packages/WXYCAPI/Sources/WXYCAPI/DTOs/`). `wxyc-ios-64` is the exception: it vendors this repo's generated Swift output directly rather than hand-authoring (see "Swift codegen uses the `swift6` generator" below). (`library-scanner`, formerly a Swift consumer, is now an archived/read-only repo.)
...
`format: date` and `format: date-time` are **not the same wire concept**, and the Swift generator must not treat them as one. `format: date-time` is a true instant — a point on the UTC timeline — and `Foundation.Date` is the correct type for it; those 43 sites are untouched by anything below. `format: date` is a **calendar date**: a day on a wall calendar with no time-of-day and no timezone (`rotation_kill_date`, `add_date`/`kill_date`, `Concert.starts_on`, and the two `GET /concerts` `from`/`to` query params). Decoding a calendar date into `Foundation.Date` fabricates a UTC-midnight instant the value never had, which silently renders a day early for any client west of UTC and re-encodes as a date-time (`"2026-08-20T00:00:00.000Z"`) instead of round-tripping the bare `"2026-08-20"` the server expects.
...
The #355 spike confirmed this generator version keys `typeMappings` on the OpenAPI `format`, not the base `type` — mapping `date` retargets exactly the 9 `format: date` schema properties + 2 query params (11 sites across 7 generated model files) and leaves all 43 `format: date-time` properties on `Date`, verified by an exhaustive diff, not sampling.
...
The behavioral test suite for `CalendarDate` (round-trip, timezone-shift regression, total ordering, malformed-input rejection, leap years, instant→day bridging) lives in `wxyc-ios-64` (WXYC/wxyc-ios-64#941), not here — that repo is `CalendarDate`'s compilation home and the only place with a Swift test runner in CI. **Do not merge a change here until that adoption PR is open and green against this repo's head SHA**; see #357's Constraints.
