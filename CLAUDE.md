...
**`generate:typescript` and `generate:swift` produce output real consumers vendor; `generate:python` and `generate:kotlin` are still local reference trees only.** TypeScript ships the DTOs in the published package; Swift's `generated/swift/` is vendored directly into **three** downstream trees, each pinned by its own `contract-version.json` (see "Swift codegen uses the `swift6` generator" below). Editing this repo's codegen flags for TypeScript or Swift is therefore a real change for a downstream repo; editing Python or Kotlin flags changes nothing until each of those gets its own migration — the "Who'd notice a change here" column is the one to read before assuming otherwise, per repo, not per language.
...
| `generate:swift` | `generated/swift/` | **wxyc-ios-64, wxyc-dj-ios, wxyc-swift-auth** | All three vendor this output directly, each regenerated against its own pinned wxyc-shared commit: ios-64 into `Shared/WXYCAPIModels/` (WXYC/wxyc-ios-64#598), dj-ios into `Packages/WXYCAPIModels/` (WXYC/wxyc-dj-ios#75, merged 2026-08-17 — ~262 files), and wxyc-swift-auth into `Sources/WXYCAuth/Generated/` (WXYC/wxyc-swift-auth#2). dj-ios keeps a handful of DTOs hand-authored alongside the vendored tree (`Packages/WXYCAPI/Sources/WXYCAPI/DTOs/`), each for a reason its CLAUDE.md records per type — a deliberate residue, not an unmigrated remainder. **They are not symmetric on `Infrastructure/`** — see the `CalendarDate` section below, which is where that asymmetry bites. **wxyc-swift-auth is the one that vendors a *subset*** — see below |
...
**One consumer vendors a subset, and the difference matters when reading a diff of this repo's Swift output.** `wxyc-ios-64` and `wxyc-dj-ios` vendor the whole of `Models/`; `wxyc-swift-auth` vendors only the 16 schemas reachable from the non-device `/auth/*` operations, demotes its `Infrastructure/` copies to `internal`, and enforces both with a `$ref` closure computed from `api.yaml` (WXYC/wxyc-swift-auth#2). That is deliberate rather than an unmigrated remainder: that package lands in *both* apps' dependency graphs alongside their own full trees, so vendoring everything would put a third public copy of every app-facing schema into each graph. Two consequences for edits here. Adding a schema `$ref`'d from an auth operation **breaks that repo's regen** until its allow-list names it — by design, since an allow-list silently drops what nobody classified. And renaming or re-scoping an auth schema is a three-consumer change, not a two-consumer one.
...
`format: date` and `format: date-time` are **not the same wire concept**, and the Swift generator must not treat them as one. `format: date-time` is a true instant — a point on the UTC timeline — and `Foundation.Date` is the correct type for it; those 53 sites are untouched by anything below. `format: date` is a **calendar date**: a day on a wall calendar with no time-of-day and no timezone. The 9 in `api.yaml` today are `CatalogExportRow.rotation_kill_date`, `RotationEntry.add_date`/`.kill_date`, `RotationRowSummary.add_date`/`.kill_date`, `Rotation.rotation_add_date`/`.rotation_kill_date`, `KillRotationRequest.kill_date`, and `Concert.starts_on`, plus the two `GET /concerts` `from`/`to` query params. `Rotation.add_date` is deliberately not on this list: WXYC/Backend-Service#2349-class — it is a raw Postgres `timestamptz` rendering, not RFC 3339 in either shape, so it carries no `format` at all (see the `#228` block in `tests/api-spec.test.ts`). Decoding a calendar date into `Foundation.Date` fabricates a UTC-midnight instant the value never had, which silently renders a day early for any client west of UTC and re-encodes as a date-time (`"2026-08-20T00:00:00.000Z"`) instead of round-tripping the bare `"2026-08-20"` the server expects.
...
The #355 spike confirmed this generator version keys `typeMappings` on the OpenAPI `format`, not the base `type` — mapping `date` retargets exactly the 9 `format: date` schema properties plus the 2 `GET /concerts` query params, 11 spec sites in all, and leaves all 53 `format: date-time` properties on `Date`. Verified by exhaustive diff, not sampling.

Downstream of those 9 spec properties the generator emits **11** `CalendarDate` property declarations across **7** model files, and the gap is worth knowing before you conclude a count is wrong: `RotationWithAlbum` is an `allOf` over `RotationEntry`, which the generator flattens, so `add_date`/`kill_date` are emitted twice. The seven files are `CatalogExportRow`, `KillRotationRequest`, `Concert`, `RotationEntry`, `Rotation`, `RotationRowSummary`, and `RotationWithAlbum`; the 2 query params land in `APIs/`, which this repo's vendoring consumers drop. Alongside them 67 `Date` property declarations are unmoved — a *composite*-expanded count, likewise not equal to the 53 spec sites, and for the same reason; unlike the two counts above, this one is not re-derived here (it needs a full `allOf` graph walk or an actual generator run, not a line count) and may itself be stale. **All of these are live numbers, not constants** — re-derive the two spec-site counts (`grep -c 'format: date$' api.yaml`, `grep -c 'format: date-time' api.yaml`) rather than trusting this paragraph. It has already gone stale once, and instructively: written against api.yaml 1.36.0 it read "9 schema properties + 2 query params (11 sites)" and "43 `format: date-time`", both of which were exactly right *then*. Ten minor versions later `format: date` had fallen to 9 and `date-time` risen to 52, so a rebase turned two correct numbers into two wrong ones with nothing failing — and it happened again: WXYC/wxyc-shared#228's `Rotation` reconciliation first pushed `format: date` sites to 10 (adding `rotation_add_date`, and for the first time counting `RotationRowSummary`'s pre-existing pair) without updating this paragraph or the model-file list below it, then a same-PR fix to `Rotation.add_date`'s wrongly-claimed format (it is a raw `timestamptz` render, not RFC 3339 — see the property's own description) dropped the schema-property count to 9. The counts are worth stating — they are what "verified by exhaustive diff, not sampling" means concretely — but they are a measurement of a moving target, and any PR that sits long enough to be rebased must re-take it.
...
### Python codegen and `format: uri` on the streaming URL fields (#428)

`api.yaml` annotates the five streaming URL fields (`spotify_url`, `apple_music_url`, `youtube_music_url`, `bandcamp_url`, `soundcloud_url`, wherever they appear — `FlowsheetEntryFields`, `FlowsheetV2TrackEntry`, `AlbumMetadata`, `StreamingLinks`, `DiscogsMatchResult`) with `format: uri`, purely as documentation. `datamodel-codegen`'s default mapping for `type: string, format: uri` is pydantic's `AnyUrl`, which validates **at construction** — and these five fields carry pre-existing malformed/wrong-shaped stored values (WXYC/Backend-Service#1710), so a naive regen would turn already-persisted rows into hard decode failures on read. TypeScript (`string | null`) and Swift (`String?`) codegen treat `format: uri` as documentation-only; **Kotlin does not** — `openapi-generator`'s kotlin generator emits `val spotifyUrl: java.net.URI? = null`, the same construction-time parse of stored values that this pin exists to avoid, so a Kotlin consumer regenerating models has to pin these five fields to `String` itself or accept decode failures on the pre-guard rows. `scripts/generate-python-models.sh` makes Python match the documentation-only consumers by running a **post-generation substitution**, `pin_streaming_url_fields_to_str()`, that rewrites exactly those five fields' generated type from `AnyUrl` back to `str`, leaving every other `format: uri` field (the archive presigned-GET `url`, the OAuth device-flow `verification_uri`/`verification_uri_complete`) untouched.

**Why a post-generation substitution and not a codegen flag.** Two `datamodel-codegen` 0.56.1 flags look like they should do this field-scoped and don't: `--type-mappings 'string+uri=string'` is documented as format-scoped but is actually document-wide — it retargets every `format: uri` field in the spec. `--type-overrides '{"Model.field": "path.To.Type"}'` is documented as `"Scoped: ... replaces specific field only"`, but that claim does not hold for format-derived types — verified empirically (2026-08-31): overriding `StreamingLinks.spotify_url` alone also flipped the unrelated `DeviceCodeResponse.verification_uri` to the override target. Neither flag can express "these five fields only," which is exactly what #428's decision requires, so the pin is a regex substitution over the generated file instead — see the function's own comment in `scripts/generate-python-models.sh` for the mechanics and the exact field-line shapes it has to handle. Any leftover unused `from pydantic import AnyUrl` (only possible if some future regen removes every other `format: uri` field) is cleaned up by the `ruff check --fix` step that already runs after this one — no import bookkeeping needed in the pin itself.

**The pin is line-anchored, so it fails open against a shape it wasn't written for — a hard AST post-condition catches that.** The sed only matches `fieldname: AnyUrl` on one line. Adding `--use-annotated` to `run_codegen` (not done today, but verified against both the pinned 0.56.1 and 0.76.0) makes a described field — all five of these carry a description — generate instead as a multi-line `Annotated[AnyUrl | None, Field(description="...")]`, which the sed never matches; `pin_streaming_url_fields_to_str()` still returns 0, silently. `verify_streaming_url_fields_pinned()`, run immediately after the pin, closes that: it parses the generated file as Python, walks every class's `AnnAssign` nodes, and for each of the five field names asserts the annotation resolves to plain `str` — a structural check, not another line-anchored pattern, so it catches any future shape the sed doesn't handle, not just the one reproduced above. A bats test (`the AST post-condition fails loudly when the sed pin no-ops`) proves this by shadowing `sed` with a passthrough stub, simulating the pin no-op-ing for any reason.

**The post-condition is an allowlist (`is it str?`), not a denylist on the token `AnyUrl` (#466).** A denylist reads the field's own annotation and resolves no indirection, so it failed open through a named `$ref` schema. `api.yaml`'s `DiscogsMatchResult` says these fields "will be migrated to use `$ref` in a future version", and this generator wraps a named scalar schema in a `RootModel` — 1.56.0 already emits `class Url(RootModel[constr(max_length=2048)])`. After that migration one of the five generates as `class StreamingUrl(RootModel[AnyUrl])` plus `spotify_url: StreamingUrl | None`: the sed doesn't match it, the annotation holds no `AnyUrl` token, and a denylist reports nothing while the field decodes as a validated `AnyUrl` anyway — precisely the outcome #428 exists to prevent. The allowlist asks instead whether the names the annotation references, minus the wrappers that don't change what it decodes as (`Optional`, `Union`, `Annotated`, `Field`), are exactly `{str}`. It therefore also rejects a narrowed `constr(...)` and a `RootModel[str]` wrapper, deliberately: a bound still validates at decode time, a wrapper needs `.root` to reach the value, and neither should land unnoticed. A bound has two spellings and the name allowlist only sees one — under `--use-annotated` a `maxLength` moves out of the type and into the metadata, as `Annotated[str | None, Field(description=…, max_length=2048)]`, whose referenced names are exactly `{str}` once the wrappers come off. `Field` cannot leave the wrapper set to fix that (every correctly pinned field would fail the moment that flag is added), so the narrowing keywords are checked directly against a closed set — `max_length`, `min_length`, `pattern` are the only ways pydantic narrows a `str`.

**The rename guard lives in the bats suite, not in the script — deliberately, and divergently from LML (#466).** Both the sed and the post-condition key on the five literal names in `STREAMING_URL_FIELDS`, so renaming one upstream makes the sed no-op on a name that isn't there while the AST walk finds nothing to report: green, with four of five pinned. Asserting presence inside the script doesn't work here, because this script takes `--input` and is exercised against minimal fixture specs that legitimately declare none of the five — and scoping the requirement to what the input declares goes vacuous on exactly the rename it's meant to catch. A rename guard can only be written against a *known* document, so it lives in `scripts/__tests__/generate-python-models.test.sh` as an assertion over this repo's own `api.yaml`, which is where a rename originates and therefore where it should fire, before any consumer regenerates.

**That guard asserts from the AnyUrl side, not the name side, because each of the five names is declared by five separate schemas** (`AlbumMetadata`, `StreamingLinks`, `FlowsheetEntryFields`, `FlowsheetV2TrackEntry`, `DiscogsMatchResult`). "Does `spotify_url: str` appear somewhere in the output" is satisfied by the four survivors of a one-schema rename while the fifth ships as `AnyUrl`; counting declarations against pinned declarations doesn't close it either, since a renamed field leaves both counts equal at four. What closes it is the complement: the set of fields that generate as `AnyUrl` **at all** must be exactly the three unrelated `format: uri` fields #428 left alone (`url`, `verification_uri`, `verification_uri_complete`). A renamed streaming field lands in that set and gets named — and so does a *new* streaming URL field added to `api.yaml` without being added to `STREAMING_URL_FIELDS`, which is the same defect arriving from the other direction. A new unrelated `format: uri` field will also fail this test; that is the point, not a nuisance — it forces the "does this need pinning?" decision at the moment someone has the context to answer it. LML's fork asserts presence inside the script instead, which is correct *there* — it has exactly one input — so this line stays divergent on purpose.

**The pin (and its post-condition) are NAME-keyed, not schema-scoped.** Both match on the field name string alone, with no notion of which schema it belongs to. A future, unrelated schema that happens to declare its own field named e.g. `spotify_url` with `format: uri` gets silently pinned to `str` too, and the post-condition would treat that as correct rather than catching an unintended scope-widening — from the field name alone, the two cases are indistinguishable. Accepted rather than fixed: closing it would mean parsing the generated file's class structure instead of matching on name, real cost against a hypothetical this specific given how distinctive the five names are.

**Enforcement lives elsewhere, deliberately — and it has now shipped.** This pin makes `format: uri` documentation-only for Python, matching TypeScript and Swift (and closing the gap Kotlin still has) — it is not where malformed streaming URLs get caught. That enforcement lives at two runtime seams, both merged 2026-08-31: the BS boundary guard (`sanitizeLookupStreamingUrls`, WXYC/Backend-Service#2351) and the LML read seam (`streaming_links` read-out, `lookup/enrichment/streaming_link_validation.py`, WXYC/library-metadata-lookup#1296; it suppresses on the way out and never rewrites the persisted row), both suppressing a malformed value to `null` rather than raising, matching the WXYC/Backend-Service#1714 and #2345 suppress-to-null policy. The two seams deliberately disagree on `bandcamp_url`: the BS boundary applies its host allowlist to spotify/apple_music/youtube_music/soundcloud and checks bandcamp for well-formedness only (custom-domain deep links defeat a host allowlist there), while the LML read seam host-checks all five and additionally floors youtube_music/bandcamp/soundcloud on well-formedness. `api.yaml` carries this as **two** YAML-anchored notes rather than 25 copies (WXYC/wxyc-shared#431): `&streaming-url-note-album` on the three deep-link fields (`spotify_url`/`apple_music_url`/`bandcamp_url`, 15 field/schema pairs) and `&streaming-url-note-search` on the two search-URL fields (`youtube_music_url`/`soundcloud_url`, 10 pairs). Two, not one, because YAML cannot concatenate scalars and the album-vs-search lead sentence is load-bearing — `StreamingResolution` carries a verdict for exactly the three deep-link services and none for the two search ones. The enforcement paragraph is word-for-word identical across both; only the lead differs, plus the bandcamp split-seam clause, which names `bandcamp_url` explicitly and appears in the album note only. Both anchors are defined at their first-in-document-order site in `FlowsheetEntryFields`, which is load-bearing (a YAML anchor must precede its aliases) — see `tests/api-spec.test.ts`'s streaming-URL suite, which asserts the anchor/alias counts against the raw spec source, since the parser expands aliases before any parsed-tree assertion can see them. **Both Python consumers (library-metadata-lookup, request-o-matic) still need the same `pin_streaming_url_fields_to_str` pin in their own, currently unmigrated `scripts/generate_api_models.sh`** as part of their own post-#428 regen PRs — this script's pin only protects a caller of *this* canonical script, and per the Output locations and consumers table above, neither Python service has migrated onto it yet.
...
**Any content change to `api.yaml` must move `info.version` (line ~9) forward.** `check-version-bump.yml` fails a PR that changes `api.yaml`'s content without also raising `info.version` — breaking → **major**, additive path/schema/field → minor, description/docs-only → patch; a version that moves *backwards* fails too (a reused number republishes an already-used identity for a different shape).

The breaking → major rule is stated here because its absence was read as permission: every breaking change before `2.0.0` shipped as a minor, including five consecutive schema deletions (1.61.0–1.65.0). That was defensible each time — those schemas were swept and had no consumer, so nothing broke in practice — but it left the scale unable to express the case where something does break. `2.0.0` is the first major, cut for `GET /flowsheet`'s retype (#485); `3.0.0` followed for `GET /flowsheet/range`'s (#487). A whitelist entry in `oasdiff-err-ignore.txt` suppresses the *gate*, not the incompatibility; if a change needs one of those lines, the version is where consumers are told. The two are a pair — neither substitutes for the other. **But do not read a quiet gate as "not breaking."** Those two retypes are the same defect and the same fix, and oasdiff classified them differently: #485 changed the response body itself from `array` to `object` and tripped ERR, while #487 only changed an item schema from `allOf` to `oneOf` *inside* a response property, which oasdiff reports as two warnings and zero errors — even though it deleted two published schemas and changed the type every consumer generates for a row. So #487 took a major with no whitelist entry at all, and the version was the only signal consumers got. The gate finds a subset of breaking changes; deciding the bump is still yours. The diff is byte-level, deliberately: a `#`-comment or whitespace edit *inside `api.yaml`* counts as content and requires a bump (`datamodel-codegen` renders descriptions into generated output, and byte-level is the only definition that never argues). A PR that leaves `api.yaml` byte-identical — edits to any other file, comments included — never trips the gate, regardless of what `info.version` says.
...
`src/analytics/listener-events.json` (documented by the hand-written `src/analytics/listener-events.schema.json`) is the cross-platform PostHog event contract for the WXYC listener apps. Both apps report into PostHog project `134292` with shared event names — platform is carried by `$lib`, not by the name — so compatibility has to live in the properties; see `WXYC-Android/docs/adr/0001-shared-posthog-project.md`. Its two consumers are `wxyc-ios-64` and `WXYC-Android`, each vendoring the file at a pinned merge SHA and converting its own hardcoded event-name-pin suite into a **contract ⊇ app** assertion. The seed is the union of those pin suites plus the five reserved DJ-replies names — not the apps' full declaration sets: iOS events outside its pin tables (`playback_heartbeat`, `stall_recovery`, the On Tour and Spotlight families, …) are not yet declared here, so a consumer drives the ⊇ check off the names its pin tables hold, and an undeclared single-platform name is a gap to fill in this file, not a consumer failure.

The contract ⊇ app rule. **This section is the normative statement of it.** Consumers cite it at the merge SHA they vendor the contract file from; they do not restate it, because a restatement is exactly what drifts — rules 3 and 4 below are deliberately stronger than the text drafted in #521, whose rule 3 (`K ⊆ keys(props)` alone, for a closed event) left rule 5's "a dropped property fails in the repo that drifted" unenforceable, and whose rule 4 gave a `vocabulary`-bound property nothing at `props[k]` to agree with. For platform `P` and an event named `E` that the app declares with wire property keys `K`:

1. exactly one entry in `events` has `name == E`;
2. that entry's `platforms` contains `P`;
3. let `props = entry.platformProperties[P]` if `platformProperties` is present, else `entry.properties`, and let `required(props)` be the keys of `props` not marked `optional`; then `required(props) ⊆ K ⊆ keys(props)`. If `entry.openProperties` is `true` only the lower bound applies: undeclared keys are allowed, but every required key must still be emitted. `K` is the set of keys the event type itself declares — sample it before the analytics client stamps its per-capture common properties (`meta.commonProperties[P]`, e.g. iOS's `build_type`) and before the PostHog SDK adds its own `$`-prefixed keys; neither is part of `K` or of `props`;
4. for each `k ∈ K` the app's type/unit/enum agrees with `props[k]` **after dereferencing its vocabulary**: when `props[k].vocabulary` names an entry `V` in the top-level `vocabulary` block, the effective `type`/`unit`/`enum` are `V`'s, with `V.platformEnum[P]` replacing `V.enum` when that key exists — so Android's `play.source`/`pause.source` are checked against `vocabulary.source.platformEnum.android` and iOS's against `vocabulary.source.enum`. A property may omit `unit`/`enum` and carry them only through `V` (`play.source` and `pause.source` do), so reading `props[k]` alone checks nothing; `resolveProperty` in `tests/listener-events.test.ts` is the reference dereference, and that suite guarantees a property never contradicts its vocabulary;
...
This file is **data only**: it touches no version number, cuts no tag, and is not gated by `check-version-bump.sh` (that gate reads `api.yaml`'s content only, and this file changing content leaves `api.yaml` byte-identical). Consumers pin this file by its **merge SHA**, not by a tag — there is no release step for it, unlike `api.yaml`'s `gha`/`vN` tags above. `meta.version` is the version of the file's *shape*, not of its contents: it moves only on an incompatible change to `listener-events.schema.json` (a renamed or re-typed field), never on a data change, so a consumer that pins `meta.version` learns nothing from an equality check after a re-vendor — the merge SHA is the data version.
