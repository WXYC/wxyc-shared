...
**Enforcement lives elsewhere, deliberately — and it hasn't landed yet.** This pin makes `format: uri` documentation-only for Python, same as every other consumer — it is not where malformed streaming URLs get caught, today or once shape validation ships. That validation is **contract-first**: it will land at the BS boundary guard (`sanitizeLookupStreamingUrls`, WXYC/Backend-Service#2350) and the LML writer seam (`streaming_links` read seam, WXYC/library-metadata-lookup#1295), both suppressing a malformed value to `null` rather than raising, matching the WXYC/Backend-Service#1714 and #2345 suppress-to-null policy — but as of this PR **neither #2350 nor #1295 has merged**. The guard that has shipped (BS#1714) covers only `spotify_url`/`apple_music_url`, not the other three fields, and only at the LML-response boundary those two issues extend — not every path these five fields reach the wire through. Don't describe this pair as live enforcement until both land; `api.yaml`'s own field descriptions and `tests/api-spec.test.ts`'s streaming-URL suite say so explicitly for exactly this reason. **Both Python consumers (library-metadata-lookup, request-o-matic) still need the same pin in their own, currently unmigrated `scripts/generate_api_models.sh`** as part of their own post-#428 regen PRs — this script's pin only protects a caller of *this* canonical script, and per the Output locations and consumers table above, neither Python service has migrated onto it yet.
