...
// ── Retired-in-this-chain auth helpers ──────────────────────────────────────
//
// `E2EAuthHelper` / `createE2EAuthHelper` / `getAnonymousToken` /
// `getAnonymousJwt` are the per-file sign-in helpers the shared-session
// machinery above replaces. They are carried unchanged here because two
// consumers still call them at this point in the chain --
// `tests/e2e-contracts.test.ts` and `e2e/auth.test.ts` -- and each moves
// off them in its own PR. The last of those PRs deletes this block.
//
// Nothing NEW should call these: every file rewired above now reads the
// session `e2e/global-setup.ts` minted once for the run.
