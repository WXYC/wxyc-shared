...
- **Consumer:** wxyc-dj-ios `AuthService.captureRotatedSessionToken` — the method name predates this correction; the value it captures is a same-token re-encoding, not a new token. It is **not** a no-op, though: the guard is a raw string compare (`rotated != sessionToken`, `AuthService.swift`), and after sign-in the stored value is the bare `<token>` while the first renewal header is `<token>.<HMAC>`, so that one renewal really does overwrite the stored credential with its signed form. Harmless — `ANONYMOUS_SIGN_IN_SHAPE` above records that the header and body token values authenticate interchangeably as a bearer — but a real write, not a skipped one. The surviving rationale for keeping the capture surface at all is wire-compatibility with a future encoding change, not "the token actually rotates"; note that is **this repo's** framing. [WXYC/wiki#121](https://github.com/WXYC/wiki/pull/121) (merged 2026-08-24) amended the wxyc-swift-auth plan to the narrower claim that the capture is "wire-hygiene rather than a liveness mechanism" — it does not use the future-encoding argument, so don't cite the plan for it. [wxyc-ios-64#970](https://github.com/WXYC/wxyc-ios-64/issues/970) closed as completed once D1 landed as a no-op-with-test ([wxyc-ios-64#995](https://github.com/WXYC/wxyc-ios-64/pull/995)). **As of 2026-08-24** two comment corrections are still open, each against text that describes rotation as real: [wxyc-dj-ios#112](https://github.com/WXYC/wxyc-dj-ios/issues/112) (the `captureRotatedSessionToken` name and its issue-#66 doc comments) and [Backend-Service#2260](https://github.com/WXYC/Backend-Service/issues/2260) (`auth.definition.ts`, inherited from the f23a45c8/BS#966 commit comment). Separately — and **not** a comment correction, so don't file it under this heading — [wxyc-ios-64#997](https://github.com/WXYC/wxyc-ios-64/issues/997) carries the orphaned-anonymous-user mitigation that #970 originally set out to deliver; it is a behavioral ticket whose real shape is a client-side keep-alive rather than a capture.
