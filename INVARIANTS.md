...
- **Consumer:** wxyc-dj-ios `AuthService.captureRotatedSessionToken` — the method name predates this correction; the value it captures is a same-token re-encoding, not a new token. Persisting it is a no-op wherever the guard already compares decoded-token equality. The surviving rationale for keeping that capture surface at all is wire-compatibility with a future encoding change — not "the token actually rotates." The wxyc-swift-auth plan was amended to say so in [WXYC/wiki#121](https://github.com/WXYC/wiki/pull/121) (merged 2026-08-24), and [wxyc-ios-64#970](https://github.com/WXYC/wxyc-ios-64/issues/970) closed as completed once D1 landed as a no-op-with-test ([wxyc-ios-64#995](https://github.com/WXYC/wxyc-ios-64/pull/995)). Three repo-local text corrections remain open, each against comments that still describe rotation as real: [wxyc-dj-ios#112](https://github.com/WXYC/wxyc-dj-ios/issues/112) (the `captureRotatedSessionToken` name and its issue-#66 doc comments), [Backend-Service#2260](https://github.com/WXYC/Backend-Service/issues/2260) (`auth.definition.ts`, inherited from the f23a45c8/BS#966 commit comment), and [wxyc-ios-64#997](https://github.com/WXYC/wxyc-ios-64/issues/997) (the orphaned-anonymous-user mitigation, whose real shape is a client-side keep-alive rather than a capture).
